...
class MultiStack:
    def __init__(self, stack_size, number_of_stacks):
        self.number_of_stacks = number_of_stacks  # 스택 총 개수
        self.array = [0] * (stack_size * self.number_of_stacks)  # 전체 스택 배열 크기
        self.sizes = [0] * self.number_of_stacks  # 스택 총 개수
        self.stack_size = stack_size  # 각 스택 크기

    def push(self, value, stack_num):
        self.assert_valid_stack_num(stack_num)
