...
class MyQueue:
    ...
    def peek(self):
        if self.is_empty():
            return False
        self._shift_stacks()
        return self.old.stack.peek()  # peek는 왜 보는 거지?

    def remove(self):
        if self.is_empty():
            return False
        self._shift_stacks()
        return self.old.stack.pop()
...
class Tests(unittest.TestCase):
    ...
    def test_size(self):  # 왜 넣었다 지우는 거지?
        for sequence in self.test_case:
            q = MyQueue()
    ...
    def test_add_simple(self):
        ...
        assert q.peek() == 4
        q.add(101)
        assert q.peek() != 101  # 101은 어디서 나온 거지?
